...
### Follow-up fixes (2026-09-18, same day) — two real gaps in the first pass of this resolution

A second-pass review of this resolution (by Duan, reading the writeup above against the actual code) caught two problems with the first pass's own design, both fixed the same day:

1. **The field-presence shortcut Gap 1 was written to avoid was still open for the other four signals.** The Gap 1 writeup above correctly identifies that `qa.verify_claim()`'s `TEXT_FIELD_POINTERS` branch matches a `"mission_text: '...'"` citation merely because `mission_text` is non-empty, without checking the quoted content is real — and that's exactly why `leadership_composition` was given its own dedicated `_citation_is_published_self_description()` check instead of calling `verify_claim()` directly. But `_citation_supports_claim()`, the function written for Gap 3's fix covering `population_served`/`mission_language`/`programming`/`funder_base` and every alignment criterion, *did* call `verify_claim()` directly — carrying the exact same loophole into the four signals hard rule 1's fix was specifically designed around. A fabricated citation like `"mission_text: 'we exclusively serve LGBTQ+ youth'"` on `population_served` would have passed as grounded simply because the org has *some* mission text, regardless of whether that specific claim was real.
   **Fix:** `_citation_supports_claim()` now branches on citation shape. A citation naming a numeric/boolean/categorical field (`NUMERIC_FIELD_POINTERS`/`BOOLEAN_FIELD_POINTERS`/`CATEGORICAL_FIELD_POINTERS` from `qa.py`) still defers to `verify_claim()` — that path already validates the actual cited *value* against the real computed signal, no loophole there. Everything else (a `mission_text:`/`program_text:` field pointer, or a bare free-text quote) is now checked directly against the real corpus via `_quoted_text_matches_corpus()`, bypassing `verify_claim()`'s presence-only shortcut entirely — the same treatment `leadership_composition` already had. Verified directly: a fabricated `mission_text:` quote is now rejected; a real quote from the actual `mission_text` is accepted; a correct numeric citation (`govt_pct: 0.59`) is accepted; a hallucinated one (`govt_pct: 0.91` against a real 0.59) is rejected.

2. **`org_context` defaulting to `None` was itself a silent-bypass risk for the guard it exists to protect.** The first pass's "backward compatibility, by design" framing (quoted above, now superseded) treated the `None` default as a harmless convenience for tests of the *other* hard rules. But for the highest-priority guard in this whole review (hard rule 1, demographic inference), a parameter that silently degrades to pre-fix behavior when omitted is a real regression risk: any future caller of `enforce_hard_rules()` that forgot to pass `org_context` wouldn't error — it would just quietly re-open the exact loophole this review closed, with no signal that anything was wrong.
   **Fix:** `org_context` is now a required positional parameter (no default). Omitting it is a `TypeError` at call time, not a silent behavior downgrade — mypy now catches a missing argument statically, and `test_org_context_is_a_required_argument_not_a_silent_bypass()` pins the runtime behavior. `run_scoring.run_scoring()` (the only production caller) already always supplies real `org_context`, so this has no effect on production behavior — it only removes an unused, risky escape hatch. The ~26 test call sites across `test_score.py`/`test_g14_comprehensive.py`/`test_g14_llm_guard_gaps.py` that previously relied on the `None` default were updated to pass a real `org_context` fixture (with citations rewritten to be genuinely grounded where the test's own point required `violations == []`) rather than left as call sites depending on the removed fallback.

**Full suite: 324/324 passing** (37/37 in `test_g14_llm_guard_gaps.py`, up from 22 in the original review — 15 new tests added directly to that file, covering both the rejected and the newly-preserved-when-grounded cases for each fix; `test_score.py` and `test_g14_comprehensive.py` unchanged in count, all 26 previously-`None`-context call sites across both updated to pass real `org_context`). Reconciles against the 309-test actual baseline at the start of this work (258 original + 22 original G1.4 gap tests + 29 from an untracked, unrelated G1.3 gap-review file already in the working tree): 309 + 15 = 324. Ruff-clean, `mypy src` clean.
